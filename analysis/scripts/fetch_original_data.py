...
# Key is expected to be in the root 'analysis' folder
SERVICE_ACCOUNT_KEY_PATH = os.path.join(ANALYSIS_DIR, "data/firebase_service_account_key.json")
...
def fetch_original_data():
    ...
    try:
        ...
        # DEBUG: Try to fetch the specific missing doc
        debug_id = "0RoL3GFCNQDh0yg8ZdMc" 
        msg_doc = assessments_ref.document(debug_id).get()
        if msg_doc.exists:
             print(f"DEBUG: Found specific doc {debug_id}!")
             print(f"DEBUG DATA: {msg_doc.to_dict().get('feedback')}")
        else:
             print(f"DEBUG: Specific doc {debug_id} NOT FOUND in 'assessments'.")
        ...
        for doc in docs:
            ...
            # Fetch ALL versions to find the missing feedback
...
def save_to_csv(data_list, filename="original_gpi_wide.csv"):
    ...
    priority = ['SessionID', 'FirebaseID', 'Timestamp', 'Version', 'Feedback', 
...
if __name__ == "__main__":
    ...
    if data:
        ...
        json_path = os.path.join(DATA_DIR, "original_gpi_dump.json")

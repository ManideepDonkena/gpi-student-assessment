...
Notably, spiritual practice and Bhagavad Gita familiarity showed large effects on Guna scores ($\eta^2 = .15-.22$, surviving Bonferroni correction at $\alpha = .006$). Hierarchical regression controlling for **Results**: Factor analysis (Promax rotation) confirmed a stable three-factor structure (KMO = .779). The GPI scales demonstrated excellent internal reliability (Cronbach’s $\alpha$ = .84–.89) and convergent validity with Big Five traits ($r$ up to .67). Ideally, Variance Inflation Factors (VIF) were all **< 2.9**, confirming statistical distinctness. Hierarchical regression analysis revealed that Guna traits explained an additional **20.7% variance** ($\Delta R^2$, $p < .001$) in spiritual inclination *after* controlling for Big Five traits and gender.

**Conclusion**: The GPI is a psychometrically robust instrument that captures **statistically distinct** dimensions of personality.
nvisible to the Big Five model, supported by an innovative implicit behavioral methodology that addresses social desirability without additional self-report measures.
...
All Guna scales demonstrated good-to-excellent internal consistency (α_Sattva = .86, α_Rajas = .84, α_Tamas = .89). Item refinement removed 12 poorly performing items, improving BFI Openness (α: .55 → .73). The KMO measure of sampling adequacy was **0.779** (Meritorious), and Bartlett's test was significant (χ² = 6841.6, *p* < .001), confirming suitability for factor analysis. Joint EFA revealed **two unique Guna factors**—a Spiritual Orientation factor (5.9% variance) and a Material Desire factor (2.3%)—that loaded exclusively on Guna items with no Big Five counterpart.
...
### 3.6 Robustness
Split-half cross-validation confirmed that criterion validity (2/3 hypotheses replicated, *r*-difference < .06) and incremental validity (all Δ*R*² > .10 in both halves) were stable across random subsamples.
